function excluirPerguntaMulti() {
    ...
    xhr.open("DELETE", "http://localhost:3000/perguntas_multi/" + encodeURIComponent(id), true);

    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
            if (xhr.status === 200 || xhr.status === 204) {
                msgBox.innerHTML = "<span style='color: green;'>Pergunta excluída com sucesso!</span>";
                document.getElementById("formExcluirMulti").reset();
